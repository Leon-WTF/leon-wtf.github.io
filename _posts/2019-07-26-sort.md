...
$T(n) = T(0) + T(n-1) + n = T(n-1) + n = n * (n + 1) /2 \approx O(n^2)$ 
...
$T(n) = T(n/9) + T(n/10) + n  \approx O(n  \log n)$ 
...
```
 ...
 $T(n) = 2*T(n/2) + n = 2*(T(n/4) + n/2) + n = 2*(2*(T(n/8) + n/4) + n/2) + n = 2^k * T(n/2^k) + k*n  \approx O(n \log n)$
